...
class VehicleSelectionBottomSheetDialogFragment(private val vehicleList: List<VehiclesTO>?,
    ...
    override fun onClick(view: View?) {
        ...
        when (view?.id) {
            R.id.llVehicle1 -> {
                if (vehicleList[0].vehiclesCount == 0) {
                    toast.show()
                } else if ((distance != null && (vehicleList[0].maxDistance < distance))) {
                    toast2.show()
                } else {
            ...
            R.id.llVehicle2 -> {
                if (vehicleList[1].vehiclesCount == 0) {
                    toast.show()
                } else if ((distance != null && (vehicleList[1].maxDistance < distance))) {
                    toast2.show()
                } else {
            ...
            R.id.llVehicle3 -> {
                if (vehicleList[2].vehiclesCount == 0) {
                    toast.show()
                } else if ((distance != null && (vehicleList[2].maxDistance < distance))) {
                    toast2.show()
                } else {
            ...
            R.id.llVehicle4 -> {
                if (vehicleList[3].vehiclesCount == 0) {
                    toast.show()
                } else if ((distance != null && (vehicleList[3].maxDistance < distance))) {
                    toast2.show()
                } else {
